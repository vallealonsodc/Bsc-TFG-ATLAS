...
Generalmente, resulta de interés acceder a la base de datos remota de Broadsea desde un administrador de bases de datos local.  Docker almacena la base de datos de Broadsea en el volumen \code{atlasdb-postgres-data} (véase \ref{cap:01.3Docker}) y crea un entorno de interoperabilidad e interconectividad a través de la WebAPI, que se aloja en un servidor Postgre (véase \ref{01.4Postgre}).
...
\begin{enumerate}
    ...
    \item Tras registrar el servidor correctamente, debe aparecer una base de datos con cinco esquemas: \code{demo\_cdm}, \code{demo\_cdm\_results}, \code{public}, \code{webapi}, \code{webapi\_security} (véase \ref{01.4Postgre}).
...
\begin{enumerate}
    ...
    \item Por ejemplo, comprobar el número de filas que recupera pgAdmin de la tabla \code{person} del esquema \code{demo\_cdm}, tal y como se muestra en la Figura \ref{fig:pgAdmin}.
    ...
    El código que se muestra en la Figura está extraído (modificado) del foro \cite{forumBroadseaDB} y está disponible en el repositorio de github del TFG en la ruta \code{environment/anexes/manual/rscript} \cite{vallealonsodc}.
